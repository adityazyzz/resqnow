...
body("mobile","Mobile must be atleast 3 characters.").isLength({min:10}),
body("dateOfBirth","Enter valid DOB.").isLength({min:6}),
body("bloodGroup","Enter valid Blood Group.").isLength({min:2}),
...
async(req,resp)=>{
    ...
    try {
        ...
        if(user){
            return resp.status(400).json({success,error : "This mobile  already exists."});
        }
...
router.post("/login",[
 ...
 body("password","Password must be atleast 5 characters.").isLength({min:5})
    ...
    try {
        ...
        if(!user){
            return resp.status(400).json({success,error : "Invalid mobile or password."}); 
        }
